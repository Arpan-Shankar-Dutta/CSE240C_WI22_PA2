...
if caches['LLC']['replacement'] in caches['LLC']:
    repl_pol = caches['LLC']['replacement']
    param = caches['LLC'][repl_pol]
    param_type = list(param.keys())[0]
    value = param[param_type]
    with open(f'replacement/{repl_pol}/{repl_pol}.cc', 'r') as f:
        src = f.readlines()
    i = 0
    while i < len(src):
        if src[i].startswith("#define") and src[i].find(param_type) != -1:
            src[i] = f'#define {param_type} {value}\n'
            break
        i+=1
    with open(f'replacement/{repl_pol}/{repl_pol}.cc', 'w') as f:
        f.writelines(src)
