...
{
    //int NUM_SET = (get_config_number() <= 2) ? 2048 : MAX_NUM_SET;
...
uint32_t CACHE::find_victim (uint32_t cpu, uint64_t inst_id, uint32_t set, const BLOCK *current_set, uint64_t PC, uint64_t paddr, uint32_t type)
{
